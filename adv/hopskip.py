    ...
    def hsja(sample, target_label, target_image):
        ...
        for j in np.arange(num_iterations):
            ...
            if stepsize_search == 'geometric_progression':
                ...
                epsilons = torch.from_numpy(
                    np.logspace(-4, 0, num=20, endpoint=True)).to(perturbed.device).float() * dist
                perturbeds = perturbed + \
                    epsilons.view((20,) + (1,) * (len(shape) - 1)) * update
...
def approximate_gradient(decision_function, sample, num_evals,
    ...
    fval = 2.0 * \
        decisions.view((decisions.shape[0],) + (1,) * len(shape)) - 1.0
...
def initialize(decision_function, sample, shape, clip_min, clip_max):
    ...
    while True:
        random_noise = clip_min + \
            torch.rand(shape).to(sample.device) * (clip_max - clip_min)
